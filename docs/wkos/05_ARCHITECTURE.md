...
Entrada
  ...
  [1]  UPLOAD HANDLER
       zip-handler.ts · image-analyzer.ts
         │
  [2]  PARSER & CSS EXTRACTOR
       html-parser.ts + css-extractor.ts → LayoutNode[] (com extractedCSS)
         │
  [3]  SECTION DETECTOR
...
services/ (camada de lógica pura — sem React)                                │
  html-parser.ts ← css-extractor.ts                                          │
  css-extractor.ts                                                           │
  section-detector.ts ← html-parser.ts                                      │
  token-resolver.ts                                                          │
  elementor-mapper.ts ← section-detector.ts, elementor-utils.ts, css-extractor.ts│
...
|---|---|---|
| `css-extractor.ts` | Extrair CSS estruturado de classes Tailwind e estilos inline | `string, Record` → `ExtractedCSS` |
| `html-parser.ts` | HTML string → árvore DOM com CSS extraído | `string` → `LayoutNode[]` |
...
| `elementor-mapper.ts` | Mapear seções para Elementor (com CSS responsivo e animações) | `Section[]` → `ElementorElement[]` |
...
interface LayoutNode {
  ...
  extractedCSS?: ExtractedCSS; // CSS estruturado extraído de classes e estilos inline
}
```
